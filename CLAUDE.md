...
- Vista 3D con cinta recorrida sobre el piso: usa la odometría WiFi, dibuja sólo mientras `line_detected=1`, corta tramos al perder la línea y agrega flechas celestes según el desplazamiento real para mostrar dirección y sentido (incluido retroceso)
...
|-------|--------------------------|------------------|----------------------|
| 2026-07-21 | robotviewer3d.h/.cpp, mainwindow.cpp | **Cinta recorrida en la Vista 3D:** cada muestra `WIFI_ODOM_DATA` proyecta (X,Y) sobre el piso (100 unidades/m) sólo mientras `line_detected=1`; la cinta se segmenta al perder/recuperar línea, ignora ruido <4 mm y saltos >3 m, mantiene el robot centrado trasladando el historial, y agrega flechas celestes cada ~0.30 m orientadas por el delta real (muestran correctamente avance o retroceso). Límite de 1200 entidades para sesiones largas. `Borrar mapa` y ACK de `RESET_ODOMETRY` limpian también la cinta 3D. | Visualizar en el modelo 3D por dónde fue recorriendo la línea, con dirección y sentido. |
...
| 2026-07-21 | mainwindow.cpp, theme.qss | **RedistribuciÃ³n adaptable de la pantalla principal:** la columna de `DATO RECIBIDO` ahora tiene una pestaÃ±a para plegarla por completo a una lengÃ¼eta de 34 px y restaurarla conservando su ancho; el espacio liberado se reparte por el `QSplitter` entre grÃ¡ficos y controles. El panel derecho puede crecer hasta 620 px, PID Balance/PID LÃ­nea/Setpoint arrancan expandidos y Control Manual/joystick tambiÃ©n es plegable (abierto inicialmente). `DATO ENVIADO` conserva su plegado independiente dentro de la consola. | Aprovechar mejor el ancho de la ventana tanto con la consola recibida visible como retraÃ­da, dando prioridad operativa a PID y joystick. |
| 2026-07-12 | robotviewer3d.h/.cpp | **Fix definitivo del modelo hundiéndose bajo el piso: el bbox del mesh ahora se lee SINCRÓNICAMENTE del .obj (`loadModelBounds`).** Causa raíz encontrada: la corrección de piso de 2026-07-10 era matemáticamente exacta (re-verificado hoy con 20.275 poses aleatorias en los 3 ejes: minY=0.6 siempre), pero dependía de que el `QBoundingVolume` publicara sus puntos implícitos — ese componente suelto (sin `view` asignada) puede no publicarlos NUNCA en Qt 6, y en ese caso `m_modelMin==m_modelMax==0` hacía que la corrección se salteara EN SILENCIO: base clavada a altura fija 25 y pivote de rotación en el origen del mesh (que NO está centrado: centro real (-2.3,-2.8,2.1), alto 22.4 unidades → a ±90° atravesaba el piso igual que antes del fix). Ahora `loadModelBounds()` parsea las líneas `v x y z` de `AutoMicro.obj` al arrancar (mismas coordenadas locales que carga QMesh) y deja bbox+centro listos desde el primer frame; el callback del QBoundingVolume queda como refinamiento y ya no se desconecta al primer disparo (los bounds pueden refinarse después). Debug y release recompilados sin errores ni warnings. | Usuario: "sigo teniendo problema con que el auto se pasa hacia debajo del suelo en el qt, solucionalo" (tercer reporte — las dos veces anteriores se atribuyó a binario viejo) |
| 2026-07-10 | mainwindow.h/.cpp, robotviewer3d.h/.cpp | **Tercer eje en la Vista 3D: banking lateral desde `lat_deg` del push de odometría.** Mirror de `WifiOdomData_t` extendido con `lat_deg` (float al final; el firmware lo llena con la inclinación lateral por acelerómetro). `setPose(pitch, yaw, lat)` con lat opcional (default 0): rotación alrededor del X local (eje de avance), orden yaw∘pitch∘lat, interpolada por la misma animación de pose; `setPitch` conserva rumbo y banking; HUD ahora muestra `Lat:`. **Si el banking sale invertido, negar `odata->lat_deg` en el handler de 0xDC de mainwindow.cpp.** El apoyo en el piso (matriz base·hijo) cubre automáticamente el nuevo eje — verificado además con test numérico standalone: minY=+0.6 exacto en ±45/±90/±120°; el "se hunde con ángulo negativo" reportado era un exe viejo corriendo (cerrar y reabrir la app tras cada rebuild). **Retrocompatible (2026-07-11)**: el handler de 0xDC acepta también el paquete SIN `lat_deg` (firmware viejo, 4 bytes menos → lat=0) — el "Packet too short" al conectar era Qt nuevo + firmware sin flashear; ahora ambas combinaciones funcionan. Compilado debug y release (Qt 6.5.2/MinGW) sin errores. | Usuario pidió que la Vista 3D reaccione también a movimientos laterales (el eje que faltaba) y reportó hundimiento en el piso con ángulos negativos |
